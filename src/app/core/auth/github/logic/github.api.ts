...
export class GithubApi {
  ...
  getUser(token: string): Observable<User> {
    ...
    return this.httpClient.get<User>(GithubApi.getBaseUri() + '/user');
  }

  getRepository(user: string, repoName: string): Observable<GithubRepo>{
    return this.httpClient.get<any>(GithubApi.getBaseUri() + `/repos/${user}/${repoName}`);
  }

  createRepository(user: string, repoName: string): Observable<GithubRepo> {
    return this.httpClient.post<any>(GithubApi.getBaseUri() + `/user/repos`, {
      name: repoName,
  ...
  createDatabase(login: string, repoName: string, filePath: string): any {
    return this.httpClient.put(GithubApi.getBaseUri() + `/repos/${login}/${repoName}/contents/${filePath}`,
      {
  ...
  getFileInfo(user: string, repoName: string, filePath: string): Observable<GithubContent> {
    return this.httpClient.get<any>(GithubApi.getBaseUri() + `/repos/${user}/${repoName}/contents/`).pipe(
